...
桌面客户端与网页版连接同一套服务。安装后首次打开，填写自己的 HTTPS 服务地址，再使用已有账号登录。“连接与更新”可更换服务；启用自动更新的客户端还可检查新版。重开应用需要重新登录。

桌面版支持 Windows、macOS 和 Linux，可用安装包以当前仓库 **Releases** 页面中公开的版本为准。构建及维护细节见 [桌面开发说明](README.development.md#桌面客户端与构建)。

### 发布自己的桌面应用

完成定制后，这套构建和发布流程可以在你自己的 GitHub 仓库运行，支持 Windows、macOS 和 Linux。可以直接让 AI 帮你准备：

> 使用 $vue3-laravel-admin，将当前项目初始化为我自己的 GitHub 仓库和应用名称，准备一个普通桌面发布版本。同步版本信息、完成检查，并告诉我如何在 GitHub 验收草稿。需要推送时先核对目标仓库。

1. 将项目保存到自己的 GitHub 仓库，让 AI 完成一次项目初始化；它会设置独立应用身份，避免接收原项目的更新。
2. 发布新版本时，让 AI 同步版本并推送版本标签。GitHub 会自动检查、构建三平台安装包，并生成待验收的草稿 Release。
3. 下载测试后，在自己的 Releases 页面公开该版本，其他人即可安装。

普通发布无需配置更新密钥。需要客户端在线更新时，再为应用配置一次签名密钥；Windows/Apple 系统证书按发行需求另行提供。首次使用 fork 可能需要在 Actions 页面启用工作流，组织策略也可能限制发布权限。

初始化命令、自动更新、私有仓库及发布排障见 [在自己的 GitHub 仓库发布](README.development.md#在自己的-github-仓库发布)。Web 服务的部署与更新仍使用前面的部署流程。
