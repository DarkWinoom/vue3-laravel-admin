...
Route::prefix('v1')->group(function () {
    ...
    Route::middleware(AuthenticateSession::class)->group(function () {
        ...
        Route::put('users/{id}', [UserController::class, 'save'])->whereNumber('id');
        Route::delete('users/{id}', [UserController::class, 'destroy'])->whereNumber('id');
        Route::get('menus', [MenuController::class, 'index']);
        Route::post('menus', [MenuController::class, 'save']);
        Route::put('menus/{id}', [MenuController::class, 'save'])->whereNumber('id');
        ...
        Route::put('{resource}/{id}', [AccessController::class, 'save'])->whereIn('resource', ['roles', 'permissions'])->whereNumber('id');
        Route::delete('{resource}/{id}', [AccessController::class, 'destroy'])->whereIn('resource', ['roles', 'permissions'])->whereNumber('id');
    });
});
