...
const { spec, cases, requestCases = [] } = JSON.parse(readFileSync(process.argv[2], 'utf8'));
...
for (const { path, method, body, valid } of requestCases) {
  const schema = spec.paths[path][method].requestBody.content['application/json'].schema;
  const validate = ajv.compile({ ...schema, components: spec.components });
  if (validate(body) !== valid) throw new Error(`Request contract mismatch: ${method} ${path}`);
}
